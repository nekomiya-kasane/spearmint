...
const lexer_info mf_info = {
    ...
    "https://www.gnu.org/software/make/", 10,
};
}

const lexer_info& makefile_lexer::info() const noexcept { return mf_info; }
...
state_map makefile_lexer::get_rules() const {
    ...
    rules["root"] = {
        ...
        {R"(\b(ifeq|ifneq|ifdef|ifndef|else|endif|include|-include|sinclude|override|export|unexport|define|endef|undefine|vpath)\b)", tk::keyword::self, state_action::none()},
        {R"(\$\([^)]+\))", tk::name::variable, state_action::none()},
        {R"(\$\{[^}]+\})", tk::name::variable, state_action::none()},
        {R"(\$[@<^+?*%])", tk::name::variable, state_action::none()},
        {R"(\$\$)", tk::name::variable, state_action::none()},
        {R"(\b(subst|patsubst|strip|findstring|filter|filter-out|sort|word|wordlist|words|firstword|lastword|dir|notdir|suffix|basename|addsuffix|addprefix|join|wildcard|realpath|abspath|error|warning|info|shell|origin|flavor|foreach|if|or|and|call|eval|file|value)\b)", tk::name::builtin, state_action::none()},
...
SPEARMINT_API void register_makefile_lexer() {
    register_lexer([]() -> std::unique_ptr<lexer> {
        return std::make_unique<makefile_lexer>();
    }, mf_info);
}
