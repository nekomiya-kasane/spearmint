...
state_map markdown_lexer::get_rules() const {
    ...
    rules["root"] = {
        ...
        {.pattern = R"(^[=-]{3,}\s*$)",
         ...
         .action = state_action::none(),
         .group_tokens = {}},
        {.pattern = R"(^>\s?[^\n]*)", .token = tk::generic::emph, .action = state_action::none(), .group_tokens = {}},
        {.pattern = R"(^```[^\n]*)",
        ...
        {.pattern = R"(\*\*[^*]+\*\*)",
         ...
         .action = state_action::none(),
         .group_tokens = {}},
        {.pattern = R"(__[^_]+__)", .token = tk::generic::strong, .action = state_action::none(), .group_tokens = {}},
        ...
        {.pattern = R"(~~[^~]+~~)", .token = tk::generic::deleted, .action = state_action::none(), .group_tokens = {}},
        {.pattern = R"(`[^`]+`)",
         .token = tk::literal::string::backtick,
         .action = state_action::none(),
         .group_tokens = {}},
        ...
        {.pattern = R"(https?://[^\s<>]+)",
         ...
         .action = state_action::none(),
         .group_tokens = {}},
        {.pattern = R"(^---\s*$)", .token = tk::punctuation::self, .action = state_action::none(), .group_tokens = {}},
