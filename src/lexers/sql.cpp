...
const lexer_info sql_info = {
    ...
    "https://en.wikipedia.org/wiki/SQL", 10,
};
}

const lexer_info& sql_lexer::info() const noexcept { return sql_info; }
...
state_map sql_lexer::get_rules() const {
    ...
    rules["root"] = {
        ...
        {R"(\b(?i)(SELECT|FROM|WHERE|INSERT|INTO|UPDATE|DELETE|CREATE|DROP|ALTER|TABLE|INDEX|VIEW|DATABASE|SCHEMA|GRANT|REVOKE|SET|SHOW|DESCRIBE|EXPLAIN|USE|BEGIN|COMMIT|ROLLBACK|SAVEPOINT|TRUNCATE|MERGE|REPLACE|CALL|EXECUTE|EXEC|WITH|RECURSIVE|UNION|INTERSECT|EXCEPT|ALL|DISTINCT|AS|ON|JOIN|INNER|LEFT|RIGHT|FULL|OUTER|CROSS|NATURAL|USING|GROUP|BY|ORDER|HAVING|LIMIT|OFFSET|FETCH|NEXT|ROWS|ONLY|ASC|DESC|CASE|WHEN|THEN|ELSE|END|IF|WHILE|LOOP|FOR|RETURN|DECLARE|CURSOR|OPEN|CLOSE|FETCH|INTO|PROCEDURE|FUNCTION|TRIGGER|EXISTS|IN|BETWEEN|LIKE|IS|NOT|AND|OR|NULL|TRUE|FALSE|VALUES|DEFAULT|PRIMARY|KEY|FOREIGN|REFERENCES|UNIQUE|CHECK|CONSTRAINT|ADD|COLUMN|RENAME|TO|CASCADE|RESTRICT|NO|ACTION)\b)", tk::keyword::self, state_action::none()},
        {R"(\b(?i)(INT|INTEGER|BIGINT|SMALLINT|TINYINT|FLOAT|DOUBLE|DECIMAL|NUMERIC|REAL|CHAR|VARCHAR|TEXT|NCHAR|NVARCHAR|NTEXT|BLOB|CLOB|DATE|TIME|TIMESTAMP|DATETIME|BOOLEAN|BOOL|SERIAL|BIGSERIAL|UUID|JSON|JSONB|XML|ARRAY|BINARY|VARBINARY|BIT)\b)", tk::keyword::type, state_action::none()},
        {R"(\b(?i)(COUNT|SUM|AVG|MIN|MAX|COALESCE|NULLIF|CAST|CONVERT|SUBSTRING|TRIM|UPPER|LOWER|LENGTH|CONCAT|REPLACE|ROUND|FLOOR|CEIL|ABS|NOW|CURRENT_DATE|CURRENT_TIME|CURRENT_TIMESTAMP|EXTRACT|DATE_PART|ROW_NUMBER|RANK|DENSE_RANK|NTILE|LAG|LEAD|FIRST_VALUE|LAST_VALUE)\b)", tk::name::builtin, state_action::none()},
...
SPEARMINT_API void register_sql_lexer() {
    register_lexer([]() -> std::unique_ptr<lexer> {
        return std::make_unique<sql_lexer>();
    }, sql_info);
}
