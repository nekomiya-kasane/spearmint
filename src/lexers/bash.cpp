...
state_map bash_lexer::get_rules() const {
    ...
    rules["root"] = {
        ...
        {.pattern = R"(#![^\n]*)", .token = tk::comment::hashbang, .action = state_action::none(), .group_tokens = {}},
        {.pattern = R"(#[^\n]*)", .token = tk::comment::single, .action = state_action::none(), .group_tokens = {}},
        {.pattern = R"(\b(if|then|else|elif|fi|for|while|until|do|done|in|case|esac|function|select|time|coproc)\b)",
